...
const save_cache = (bucket, key, paths, overwrite) => __awaiter(this, void 0, void 0, function* () {
    try {
        ...
        if (!overwrite) {
            try {
                ...
                yield exec.exec(`tar cpzf ${cache_file} ${paths} -P`);
                console.log('Uploading cache to Google Cloud Storage...');
                yield exec.exec(`gsutil -o GSUtil:parallel_composite_upload_threshold=50M cp -r ${cache_file} gs://${bucket}`);
            }
        }
        else {
            console.log(`Compressing cache to ${cache_file}`);
